...
(function () {
  var cdn = '/lib/MathJax/MathJax.js?config=TeX-AMS-MML_SVG-full,local/local';
  ...
  CKEDITOR.plugins.add('mathjax', {
    ...
    init: function (editor) {
      ...
      editor.widgets.add('mathjax', {
        ...
        mask: true,
        allowedContent: 'latex(*);span(*)',
        pathName: editor.lang.mathjax.pathName,

        template: '<latex class="' + cls + '" style="display:inline-block" data-cke-survive=1></latex>',
        ...
        hasData: function () {
          return this.data.math || false;
        },

        upcast: function (el, data) {
          var isLatex = el.name == 'latex';
          var isSpan = el.name == 'span' && el.hasClass(cls);
          if (!isLatex && !isSpan) {
            return;
          }
          if (isSpan) {
          ...
          if (attrs.style) { attrs.style += ';display:inline-block'; } else { attrs.style = 'display:inline-block'; }
  ...
  CKEDITOR.plugins.mathjax.copyStyles = function (from, to) {
    ...
    for (var i = 0; i < stylesToCopy.length; i++) {
      ...
      var val = from.getComputedStyle(key);
  ...
  if (!(CKEDITOR.env.ie && CKEDITOR.env.version == 8)) {
    CKEDITOR.plugins.mathjax.frameWrapper = function (iFrame, editor) {
      var buffer; var preview; var value; var newValue;
      ...
      var updateDoneHandler = CKEDITOR.tools.addFunction(function () {
        ...
        /**
                     * 添加附加高度，解决部分公式被截取的问题.
                     * chenzuopeng on 15-8-5
                     */
        var height = Math.max(doc.$.body.offsetHeight, doc.$.documentElement.offsetHeight) + 5;

        var	width = Math.min(preview.$.offsetWidth, doc.$.body.scrollWidth);
        iFrame.setStyles({
          height: height + 'px',
          width: width + 'px'
        });
        ...
        if (value != newValue) { update(); } else { isRunning = false; }
      });
