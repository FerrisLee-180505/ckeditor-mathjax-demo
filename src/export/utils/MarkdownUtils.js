...
export function htmlToMarkdown(htmlText) {
  ...
  const nextMarkdownText = converter.makeMd(template)
  return reduce(latexContent, (rs, value, key) => {
    return rs.replace(key, `[math]${value.slice(2, value.length - 2)}[/math]`)
  }, nextMarkdownText)
}
export function markdownToHtml(markdownText) {
  // 1.we should convert Latex into <span class="math-tex">xxx</span>
